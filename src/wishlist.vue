    ...
    <div class="carousels">
      <h3>My Favourite</h3 >
        <div v-if="wishlist.length === 0">
          <p>No items in the Favourite</p>
        </div>
        <div v-else>
      <div class="carousels-slider">
          <div  class="carousels-list">
              <div v-for="(item, index) in wishlist" :key="index" class="carousels-item">
                  <img :src="'https://image.tmdb.org/t/p/original/' + item.data.backdrop_path" alt="Backdrop Image">
                  <div class="image-text">
                    <p :class="{ 'dark-mode': darkMode }">
                      <i class="bi bi-heart-fill" @click="removeFromWishlist(index)"></i>
...
export default {
  ...
  methods: {
    ...
    async removeFromWishlist(index) {
      const itemId = this.wishlist[index].id; 
      try {
