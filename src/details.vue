<template>
    ...
    <div class="library">
        ...
        >
            <div class="inner">
              <div class="banner">
                <div class="title">
                  <h3 :class="{ 'dark-mode': darkMode }">{{ showDetails.original_title }}</h3>      
                  </div>
                  <div class="btn">
                    <button class="trailerbtn">
                      <a :href="'https://www.youtube.com/watch?v=' + getTrailerKey()" target="_blank">
        ...
        <div  :class="{ 'dark-mode': darkMode }" class="movie-detail">
            <h1>Story Line</h1>
            <div class="detail">
              <p class="para">{{ showDetails.overview }}</p>
            </div>
          </div>
  
            <div class="carousels">
              <h3><i :class="{ 'dark-mode': darkMode }"></i> Top Cast</h3 >
              <div class="carousels-slider">
                <div class="carousels-list">
                
                  <div v-for="castMember in showDetails.cast" :key="castMember.id" class="carousels-item">
                    
                    <img :src="'https://image.tmdb.org/t/p/original/' + castMember.profile_path" alt="Cast Member" :title="castMember.name">
                
                    <div :class="{ 'dark-mode': darkMode }" class="image-text">{{ castMember.name }}</div>
                    <div :class="{ 'dark-mode': darkMode }" class="image-text">{{ castMember.character }}</div>
                  </div>
                </div>
              </div>
            </div>
  
            <div class="trailer-sidebar">
              <div class="trailer-header">
                <h2 class="trailer-title">Trailers</h2>
              </div>
              <div class="trailer-container">
                <iframe class="trailer-iframe" :src="'https://www.youtube.com/embed/' + getTrailerKey(0)" frameborder="0" allowfullscreen></iframe>
              </div>
              <div class="trailer-container">
                <iframe class="trailer-iframe" :src="'https://www.youtube.com/embed/' + getTrailerKey(1)" frameborder="0" allowfullscreen></iframe>
              </div>
              <div class="trailer-container">
                <iframe class="trailer-iframe" :src="'https://www.youtube.com/embed/' + getTrailerKey(2)" frameborder="0" allowfullscreen></iframe>
              </div>
            </div>
  </template>
    ...
    mounted() {
      ...
      this.fetchShowDetails(showId)
        ...
        .catch(error => {
          ...
          this.isPageLoaded = true; // Mark the page as loaded even if there's an error
        });
    },
    methods: {
      async fetchShowDetails(id) {
        try {
          const [detailsResponse, creditsResponse] = await Promise.all([
            fetch(`https://api.themoviedb.org/3/movie/${id}?api_key=843e1637e2566935058ea3d9a03a551f&append_to_response=videos`),
            fetch(`https://api.themoviedb.org/3/movie/${id}/credits?api_key=843e1637e2566935058ea3d9a03a551f`)
          ]);
      ...
      getTrailerKey(index) {
        if (this.showDetails && this.showDetails.videos && this.showDetails.videos.results.length > index) {
          return this.showDetails.videos.results[index].key;
        }
        return '';
      },
  ...
  .movie-detail {
    width: 100vw;
    height: calc(100vh - (height of your header and other fixed elements));
    position: relative;
  }
